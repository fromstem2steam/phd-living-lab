...
def render_live_order_status(order_id):
    ...
    origins = (
        supabase.rpc("get_order_all_origins", {"target_order_id": order_id})
        .execute()
        .data
    )
    if origins:
        ...
        for orig in origins:
            st.success(
                f"**{orig['product_name']}** uses **{orig['crop_name']}** from **{orig['wall_section']}**"
            )
            for r in range(1, 4):
                cols = st.columns(3)
                for c in range(1, 4):
    ...
    if st.session_state.cart:
        ...
        if st.button("Submit & Pay Order 🚀", type="primary"):
            ...
            for item in st.session_state.cart:
                ...
                supabase.schema("ordering").table("order_items").insert(
                    payload
                ).execute()
