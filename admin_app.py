...
with tab1:
    ...
    items = supabase.table("retail_inventory").schema("ordering").select("*").order("name").execute().data
    
    if items:
        df = pd.DataFrame(items)
        st.dataframe(df[["sku", "name", "price", "stock_quantity", "is_active"]], use_container_width=True)
        
        st.divider()
        st.subheader("Quick Stock Adjustment")
        c1, c2, c3 = st.columns(3)
        selected_item = c1.selectbox("Select Product", options=items, format_func=lambda x: f"{x['name']} (SKU: {x['sku']})")
        new_qty = c2.number_input("New Stock Quantity", value=selected_item["stock_quantity"], min_value=0)
        
        if c3.button("Update Stock"):
            supabase.table("retail_inventory").schema("ordering").update({
    ...
    with st.form("new_product_form"):
        ...
        name = st.text_input("Product Name (e.g., Botanical Extraction Tonic)")
        desc = st.text_area("Description")
        price = st.number_input("Price (€)", min_value=0.0, value=12.50, step=0.50)
        stock = st.number_input("Initial Stock Quantity", min_value=0, value=20)
        
        if st.form_submit_button("Save Product") and sku and name:
            qr_id = f"ITEM-{sku}"
            supabase.table("retail_inventory").schema("ordering").insert({
                "sku": sku,
                "name": name,
                "description": desc,
                "price": price,
...
with tab2:
    ...
    locations = supabase.table("wall_locations").schema("hydroponics").select("id").execute().data
    ...
    with st.form("batch_form"):
        ...
        if st.form_submit_button("Register Active Harvest Batch") and crop_name:
            supabase.table("crop_batches").schema("hydroponics").insert({
                "crop_name": crop_name,
                "location_id": selected_loc,
                "is_active_harvest": True
            }).execute()
            st.success(f"Batch '{crop_name}' assigned to pod {selected_loc}!")
            st.rerun()
            
    st.divider()
    st.subheader("Active Wall Batches")
    active_batches = supabase.table("crop_batches").schema("hydroponics").select("*").eq("is_active_harvest", True).execute().data
    if active_batches:
        st.dataframe(pd.DataFrame(active_batches)[["crop_name", "location_id", "planted_at"]], use_container_width=True)
...
with tab3:
    ...
    batches = supabase.table("crop_batches").schema("hydroponics").select("id, crop_name, location_id").eq("is_active_harvest", True).execute().data
    ...
    with col_log:
        ...
        if not batches:
            st.info("No active crop batches found. Register a batch in Tab 2 first.")
        else:
            with st.form("health_inspection_form"):
                ...
                growth_stage = st.selectbox("Growth Stage", ["Germination", "Seedling", "Vegetative", "Harvest-Ready"])
                health_score = st.slider("Overall Health Rating (1-10)", 1, 10, 8)
                canopy_height = st.number_input("Canopy Height (cm)", min_value=0.0, value=5.0, step=0.5)
                leaf_condition = st.selectbox("Leaf Condition", ["Healthy Green", "Yellowing (Chlorosis)", "Tip Burn", "Wilting"])
                pest_status = st.selectbox("Pest/Disease Status", ["None", "Aphids", "Spider Mites", "Algae Build-up", "Root Rot"])
                notes = st.text_area("Inspection Notes / Remediation Taken")
                inspector = st.text_input("Inspector Name / Youth ID", value="Youth Cohort #1")
                
                if st.form_submit_button("Submit Health Log 📝"):
                    supabase.table("plant_health_logs").schema("hydroponics").insert({
                    ...
                    st.success(f"Health log saved for {selected_batch['crop_name']}!")
                    st.rerun()
    ...
    with col_viz:
        ...
        if batches:
            ...
            logs = supabase.table("plant_health_logs").schema("hydroponics").select("*").eq("batch_id", view_batch["id"]).order("created_at").execute().data
            
            if logs:
                df_logs = pd.DataFrame(logs)
                fig_height = px.line(
                    df_logs, x="created_at", y="canopy_height_cm", 
                ...
                st.metric("Latest Health Score", f"{latest_log['health_score']}/10", delta=f"Stage: {latest_log['growth_stage']}")
...
with tab4:
    ...
    telemetry_data = supabase.table("telemetry_logs").schema("hydroponics").select("*").order("created_at", desc=True).limit(50).execute().data
    ...
    if telemetry_data:
        ...
        fig = px.line(tdf, x="created_at", y=["ph", "ec", "water_temp"], title="Sensors Trajectory Over Time")
    ...
    training_data = supabase.table("module_completions").schema("training").select("*").execute().data
    if training_data:
